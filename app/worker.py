...
class Worker:
    def __init__(self, settings: Settings) -> None:
        ...
        self.data = TwelveDataClient(settings.twelve_data_api_key.get_secret_value())
    ...
    async def run(self) -> None:
        ...
        tasks = [
            ...
            asyncio.create_task(self.telegram_command_loop(), name="telegram-commands"),
        ]
        if self.settings.enable_websocket:
            tasks.append(asyncio.create_task(self.websocket_loop(), name="websocket"))
    ...
    async def scan_loop(self) -> None:
        ...
        while not self.stop_event.is_set():
            ...
            LOGGER.info("Next M15 scan in %.0f seconds", delay)
            try:
            ...
            if self.stop_event.is_set():
                return
            if self._forex_market_window(datetime.now(timezone.utc)):
                await self.scan_once()
            else:
                LOGGER.info("Forex market window is closed; scan skipped")

    async def scan_once(self) -> None:
        LOGGER.info("Starting M15 scan for %d symbols", len(self.symbol_config))
        ...
        m5_budget = 2  # Basic 8 plan: six M15 calls + at most two M5 calls/minute.
        ...
        for symbol, m15 in frames.items():
            ...
            if evaluation.needs_m5_confirmation and m5_budget > 0:
                ...
                try:
                    ...
                    LOGGER.warning("M5 confirmation failed for %s: %s", symbol, exc)
            evaluations[symbol] = evaluation
            await self._handle_evaluation(evaluation)

            # Candle-close fallback for tracked trades if WebSocket is unavailable.
            if not self.websocket_available:
                await self._process_price(symbol, float(m15["close"].iloc[-1]))

        LOGGER.info(
            "Scan complete: %s",
            ", ".join(
                f"{symbol}={evaluation.score}/6" for symbol, evaluation in evaluations.items()
            ),
        )
    ...
    async def _handle_evaluation(self, evaluation: Any) -> None:
        ...
        if signal is None:
            return
        existing = await self.state.get_signal(signal.signal_id)
        decimals = int(self.symbol_config[signal.symbol]["price_decimals"])
    ...
    async def websocket_loop(self) -> None:
        try:
            async for event in self.data.stream_prices(
                self.symbol_config.keys(), stop_event=self.stop_event
            ):
                symbol = str(event.get("symbol", ""))
                price_raw = event.get("price")
                if symbol and price_raw is not None:
                    self.websocket_available = True
                    await self._process_price(symbol, float(price_raw))
        except asyncio.CancelledError:
            raise
        except Exception as exc:
            self.websocket_available = False
            LOGGER.exception("WebSocket unavailable: %s", exc)
            await self.telegram.send_message(
                "⚠️ Live WebSocket tracking is unavailable.\n\n"
                "Signal scans will continue on completed M15 candles, but entry/SL/TP "
                "alerts can be delayed until WebSocket access is restored."
            )

    async def _process_price(self, symbol: str, price: float) -> None:
        for tracked in await self.state.active_signals():
            if tracked.symbol != symbol:
                continue
            now = datetime.now(timezone.utc)
            if now >= tracked.expires_at and not tracked.entry_hit:
            ...
            if not tracked.entry_hit:
                invalid = (
                    tracked.direction == Direction.BUY and price <= tracked.stop_loss
                ) or (
                    tracked.direction == Direction.SELL and price >= tracked.stop_loss
                )
                if invalid:
                    tracked.status = SignalStatus.INVALIDATED
                    tracked.tracked = False
                    await self.state.save_signal(tracked)
                    await self.telegram.send_message(format_invalidation_alert(tracked))
                    continue

                entry_hit = (
                    tracked.direction == Direction.BUY and price <= tracked.entry
                ) or (
                    tracked.direction == Direction.SELL and price >= tracked.entry
                )
                if entry_hit:
                    tracked.entry_hit = True
                    tracked.status = SignalStatus.ENTERED
                    await self.state.save_signal(tracked)
                    await self.telegram.send_message(format_entry_alert(tracked))
                continue
            ...
            if stop_hit:
                ...
                tracked.tracked = False
                await self.state.save_signal(tracked)
                await self.telegram.send_message(format_stop_alert(tracked))
                continue

            targets = (
                ("tp1", tracked.tp1, tracked.tp1_hit, SignalStatus.TP1),
                ("tp2", tracked.tp2, tracked.tp2_hit, SignalStatus.TP2),
                ("tp3", tracked.tp3, tracked.tp3_hit, SignalStatus.TP3),
            )
            for name, level, already_hit, status in targets:
                reached = (
                    tracked.direction == Direction.BUY and price >= level
                ) or (
                    tracked.direction == Direction.SELL and price <= level
                )
                if reached and not already_hit:
                    setattr(tracked, f"{name}_hit", True)
                    tracked.status = status
                    if name == "tp3":
                        tracked.tracked = False
                    await self.state.save_signal(tracked)
                    await self.telegram.send_message(format_target_alert(tracked, name))

    async def telegram_command_loop(self) -> None:
        stored = await self.state.get_setting("telegram_update_offset")
        offset = int(stored) if stored else None
        async for update_id, text in self.telegram.command_messages(initial_offset=offset):
            await self.state.set_setting("telegram_update_offset", str(update_id + 1))
    ...
    async def _handle_command(self, text: str) -> None:
        ...
        if command == "/track":
            if len(parts) != 2:
                await self.telegram.send_message("Usage: /track EURUSD")
                return
            signal = await self.state.latest_a_for_symbol(parts[1])
            if not signal:
                await self.telegram.send_message(
                    f"No current A setup found for {parts[1].upper()}."
                )
                return
            signal.tracked = True
            signal.status = SignalStatus.TRACKING
            await self.state.save_signal(signal)
            await self.telegram.send_message(format_tracking_alert(signal))
        elif command == "/status":
            active = await self.state.active_signals()
            if not active:
                await self.telegram.send_message("No signals are currently being tracked.")
                return
            lines = ["📊 TRACKED SIGNALS", ""]
            for item in active:
                lines.append(
                    f"{item.symbol} {item.direction.value} — {item.status.value.upper()}"
                )
            await self.telegram.send_message("\n".join(lines))
        elif command in {"/start", "/help"}:
            await self.telegram.send_message(
                "Commands:\n"
                "/track EURUSD — track the latest A setup for that pair\n"
    ...
    def _seconds_until_next_scan(self) -> float:
        ...
        run_at = boundary + timedelta(seconds=self.settings.scan_grace_seconds)
        return max(1.0, (run_at - now).total_seconds())

    @staticmethod
    def _forex_market_window(now: datetime) -> bool:
